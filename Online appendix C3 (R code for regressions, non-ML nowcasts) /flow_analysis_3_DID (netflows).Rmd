...
#remove yandex trends
  filter(!is.na(Netflow))  # Remove rows where Netflow is NA, if any
```
...
# You don't need to do anything for columns like "Region", "Year", "Netflow", "poverty_rate", etc.
```
...
Output folder
```{r}
output_directory <- "/Users/michelle/Documents/thesis/MODEL OUTPUT/NETFLOWS" 
```
...
# Create the formula for the model (Netflow as the dependent variable) without an intercept
formula <- as.formula(paste("Netflow ~ ",   # Remove the intercept with "0 +"
...
# Create the formula for the model (Netflow as the dependent variable) without an intercept
formula <- as.formula(paste("Netflow ~ 0 +",   # Remove the intercept with "0 +"
...
fitted_vs_actual <- data.frame(
  ...
  Actual_Netflow = log(region_year_data$Netflow),  # Actual Netflow values
  Fitted_Netflow = log(expm1(fitted(full_model)))  # Convert log1p back to original scale
)
...
# Create the formula for the model (Netflow as the dependent variable)
formula <- as.formula(paste("Netflow ~ ",  
                            paste(names(region_year_data)[!(names(region_year_data) %in% c('Region', 'Year', 'Netflow'))], collapse = " + ")))
...
# Identify all potential covariates (excluding 'Region', 'Year', and 'Netflow')
all_covariates <- names(region_year_data)[!(names(region_year_data) %in% c('Region', 'Year', 'Netflow'))]
...
```{r}
extended_time_series_flows <- read_csv("/Users/michelle/Documents/thesis/extended_time_series_NETflows_noeconomiccovariates.csv") %>%
...
# Retain actual Netflow data where available, otherwise use nowcasted values
region_year_data <- region_year_data %>%
  left_join(extended_time_series_flows %>% dplyr::select(Year, Region, `Fitted Values`), 
            by = c("Year", "Region")) %>%
  mutate(Netflow = coalesce(Netflow, `Fitted Values`)) %>%
  dplyr::select(-`Fitted Values`, -Inflow, -Outflow)
```
...
Output folder
```{r}
output_directory <- "/Users/michelle/Documents/thesis/MODEL OUTPUT (w nowcasted series)/NETFLOWS" 
```
