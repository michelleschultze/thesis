...
```{r}
# Step 1: Prepare predictors (exclude non-predictors)
data_clean <- merged_data %>%
  select(-Region, -Year, -Inflow, -Outflow, -Netflow)

# Convert categorical Region to dummy variables (fixed effects)
...
Tuning the model:
```{r}
# Step 3: Grid search over α and λ using cross-validation
...
ggplot(results, aes(x = alpha, y = log(lambda), color = log(RMSE))) +
  ...
  scale_color_viridis(option = "D") +  # Apply the viridis color scale
  geom_point(data = min_rmse_point, aes(x = alpha, y = log(lambda)), 
             color = "black", size = 5, shape = 1) +  # Add a circle around the minimum RMSE point
  geom_label(data = min_rmse_point, aes(x = alpha, y = log(lambda), label = label_text),
             color = "black", fill = "white", alpha = 0.5, 
             vjust = -0.15, hjust = 0.95, label.size = 0.3) +  # Add label with background and transparency
...
# Save the ggplot as a high-resolution PNG
...
```{r}
# Step 4: Train final model on all data (2021-2023)
X_all <- merged_data %>%
  select(-Region, -Year, -Inflow, -Outflow, -Netflow) %>%
  mutate(across(everything(), as.numeric)) %>%
  as.matrix()
...
# Identify dropped variables (coefficients exactly zero)
dropped_vars <- coef_df %>% filter(Coefficient == 0)

# Identify nonzero coefficients (selected variables)
...
Save the model for use in other .Rmd files:
```{r}
# Save the full model as an RDS file
...
# Now, filter for the selected regions
...
# Plot residuals for each region as a classic residual plot
ggplot(subset_data, aes(x = residuals, y = as.factor(Region), color = Region)) + 
  geom_point(size = 3) +  # Scatter plot for residuals
  ...
  geom_text(aes(label = round(residuals, 2)), size = 3, vjust = -1) + # Label the residuals
  geom_vline(xintercept = 0, linetype = "dashed", color = "red", size = 1)  # Add a dashed line at residuals = 0
