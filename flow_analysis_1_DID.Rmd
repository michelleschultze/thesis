...
region_year_data_copy <- region_year_data %>%
  select(-mediazona) #needs to be logged only
region_year_data <- region_year_data %>%
  select(-Outflow, -Netflow) %>%
  select(-contains("mediazona"))
```
...
robust_se <- vcovHC(full_model, type = "HC0")  # HC0 for robust SE
...
# Obtain robust standard errors
robust_se <- vcovHC(stepwise_model, type = "HC0")  # HC0 for robust SE
...
```{r}
extended_time_series_flows <- read_csv("Nowcast output (Inflows)/extended_time_series_flows.csv") %>%
  mutate(across(.cols = -c(Year, Region), .fns = ~ if_else(. < 0, 0, .)))
region_year_data <- read_csv("region-year-data.csv") %>%
  filter(Year > 2018) %>%
  select(-Inflow) %>%
  select(-contains("count_"), -contains("pct_"), -contains("ratio_")) 
region_year_data['Inflow'] <- extended_time_series_flows['Fitted Values']
```
...
# Remove columns with missing first observations
region_year_data <- region_year_data[, !(names(region_year_data) %in% c("lag_minwagevalue", "lag_log_minwagevalue", "Netflow", "Outflow", "poverty_rate", "change_in_real_avg_wages", "log_minwagevalue", "lag_log_minwagevalue"))]
...
```{r}
print(region_year_data['Inflow'])
print(extended_time_series_flows['Fitted Values'])
```
